 ...
 * Aucun style inline sauf les valeurs calculées (positions, custom properties).
 * Les event listeners sont attachés séparément par _attachNodeListeners().
 */
...
const WfdComponents = (() => {
  ...
  function nodeHeader(nodeData, fam, detail) {
    return `<div class="wfd-node-header">
      ...
      <div class="wfd-flex1-min0">
        ...
        <div class="wfd-node-family">${escHtml(fam.label)}${detail.sub ? ' \u00b7 ' + detail.sub : ''}</div>
      </div>
    </div>`;
  }
  ...
  function nodePorts(ports, fam, nodeData) {
    const portH   = Math.max(ports.outputs.length, ports.inputs.length || 1) * 22 + 12;
    const inputs  = ports.inputs.map((p, i)  => port(p, 'in',  i, fam.color,            nodeData.id)).join('');
    const outputs = ports.outputs.map((p, i) => port(p, 'out', i, p.color || fam.color, nodeData.id)).join('');
    return `<div class="wfd-node-ports" style="height:${portH}px;position:relative;">
      ${inputs}${outputs}
    </div>`;
  }

  function port(portData, type, idx, color, nodeId) {
    const title = escHtml(portData.label) + (type === 'out' ? ' \u2014 Glisser pour connecter' : '');
