...
function setMode(m){
  ['image','config','designer'].forEach(function(x){
    ...
    document.getElementById('tb-'+x).style.display=x===m?'flex':'none';
    document.getElementById('mbtn-'+x).classList.toggle('active',x===m);
  });
  curMode=m;if(m==='config')cfg_load({preset:'demo'});
}
document.addEventListener('DOMContentLoaded',function(){['config','designer'].forEach(function(x){document.getElementById('mode-'+x).style.display='none';});});
...
function img_load(e){var f=e.target.files[0];if(!f)return;var url=URL.createObjectURL(f);var im=document.getElementById('arbo-img');im.onload=function(){ILoaded=true;document.getElementById('img-empty').style.display='none';im.style.display='block';document.getElementById('mm-img').src=url;document.getElementById('img-hint').textContent=f.name;img_fit();toast('Image : '+f.name);};im.src=url;e.target.value='';}
...
function img_toggleMM(){IMMon=!IMMon;document.getElementById('img-minimap').style.display=IMMon?'block':'none';document.getElementById('btn-mm').classList.toggle('on',IMMon);}
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  wrap.addEventListener('mousedown',function(e){if(e.target.closest('.dsn-node')||e.target.closest('#dsn-export'))return;if(dConnFrom){dCancelConn();return;}if(e.button!==0)return;if(e.shiftKey){var r=wrap.getBoundingClientRect();dLasso=true;dLassoX0=(e.clientX-r.left-DTX)/DS;dLassoY0=(e.clientY-r.top-DTY)/DS;dLassoSel=[];lel.style.display='block';lel.style.left=dLassoX0+'px';lel.style.top=dLassoY0+'px';lel.style.width='0';lel.style.height='0';e.preventDefault();return;}
  ...
  document.addEventListener('mousemove',function(e){
    ...
    if(dConnFrom){
      ...
      if(fromEl){var fp=dPortPos(fromEl,dConnFrom.side);var tx2=snapPort?snapPort.px:e.clientX;var ty2=snapPort?snapPort.py:e.clientY;var path2=dOrthoScreenPath(fp.x,fp.y,tx2,ty2,dConnFrom.side,snapPort?snapPort.side:null);connSvg.style.display='block';connSvg.innerHTML='<path d="'+path2+'" fill="none" stroke="#00d4aa" stroke-width="2" stroke-dasharray="5 3"/>';dConnFrom._snapPort=snapPort||null;}
    }
  });
  document.addEventListener('mouseup',function(e){
    ...
    if(dLasso){dLasso=false;lel.style.display='none';if(dLassoSel.length)toast(dLassoSel.length+' noeud'+(dLassoSel.length>1?'s':'')+' selectionne'+(dLassoSel.length>1?'s':'')+' · Suppr pour supprimer');}
  });
...
function dRenderNode(node){
  ...
  var folder=document.createElement('div');folder.className='dsn-folder';folder.style.borderLeftColor=color;folder.style.setProperty('--tab-color',color);
...
function dStartConn(id,side){dConnFrom={id:id,side:side};document.getElementById('conn-svg').style.display='block';toast('Port arrivee ou Echap pour annuler');}
function dCancelConn(){dConnFrom=null;document.getElementById('conn-svg').style.display='none';document.getElementById('conn-svg').innerHTML='';}
...
function dUpdateExport(){var paths=dBuildPaths();var panel=document.getElementById('dsn-export');var prev=document.getElementById('dsn-preview');if(!paths.length){panel.style.display='none';return;}panel.style.display='block';prev.innerHTML=paths.map(function(p){return'<div style="color:#555;font-size:9px;line-height:1.8">'+p+'</div>';}).join('');}
function dsn_showExport(){var paths=dBuildPaths();if(!paths.length){toast('Dessinez une arborescence');return;}var panel=document.getElementById('dsn-export');panel.style.display=panel.style.display==='none'?'block':'none';}
function dsn_doImport(){var paths=dBuildPaths();if(!paths.length)return;loadData();var existingPaths=visGetColPaths();var existing=collectionsData.collections||[];var added=0;paths.forEach(function(p){if(!existingPaths.includes(p)){existing.push(p);existingPaths.push(p);added++;}});collectionsData.collections=existing;localStorage.setItem('collectionsData',JSON.stringify(collectionsData));document.getElementById('dsn-export').style.display='none';toast(added+' collection'+(added>1?'s':'')+' importee');}
function dsn_clear(){dSnapshot();dNodes=[];dEdges=[];dSel=null;var vp=document.getElementById('dsn-vp');Array.from(vp.children).forEach(function(c){if(c.tagName!=='svg')c.remove();});document.getElementById('dsn-svg').innerHTML='';document.getElementById('dsn-export').style.display='none';toast('Designer vide');}
...
function dsn_checkDraft() {
  ...
  if (btn) btn.style.display = raw ? 'inline-flex' : 'none';
} 
}
function dsn_exportExcel(){
  if (!window.askidaExportExcel){ toast('Export Excel indisponible'); return; }
